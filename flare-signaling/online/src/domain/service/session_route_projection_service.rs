...
use crate::domain::event::{SessionCreatedEvent, QualityChangedEvent, SessionKickedEvent};
use flare_signaling_route::domain::DeviceRoute;
use flare_signaling_route::domain::DeviceRouteRepository;

pub struct SessionRouteProjectionService {
    device_route_repo: Arc<dyn DeviceRouteRepository>,
}

impl SessionRouteProjectionService {
    pub fn new(device_route_repo: Arc<dyn DeviceRouteRepository>) -> Self {
        Self { device_route_repo }
    }

    /// 当新 Session 被创建时，将其投影为 DeviceRoute
    pub async fn on_session_created(&self, event: &SessionCreatedEvent, session: &Session) -> Result<()> {
    ...
    pub async fn on_quality_changed(&self, event: &QualityChangedEvent, session: &Session) -> Result<()> {
        let quality_score = session.quality_score();
    ...
    pub async fn on_session_kicked(&self, event: &SessionKickedEvent) -> Result<()> {
        debug!(
            user_id = %event.user_id.as_str(),
            device_id = %event.device_id.as_str(),
            reason = %event.reason,
