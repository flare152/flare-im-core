...
pub async fn initialize(
    ...
    // 4. 创建会话仓储
    let session_repo: Arc<dyn crate::domain::repository::SessionRepository> = if let Some(ref pool) = postgres_pool {
        ...
        Arc::new(RedisSessionRepository::new(redis_client.clone(), session_config.clone()))
    };
    ...
    let message_provider: Option<Arc<dyn MessageProvider + Send + Sync>> = {
        ...
        let storage_discover = flare_im_core::discovery::create_discover(&storage_reader_service)
            .await
            .map_err(|e| anyhow::anyhow!("Failed to create storage reader service discover for {}: {}", storage_reader_service, e))?;
    ...
    let message_provider_for_domain: Option<Arc<dyn MessageProvider>> = message_provider.clone()
