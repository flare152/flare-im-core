...
impl OfflinePushSender for FcmOfflinePushSender {
    async fn send(&self, task: &PushDispatchTask) -> Result<()> {
        ...
        let fcm_token = task.metadata.get("fcm_token")
            .ok_or_else(|| ErrorBuilder::new(
        ...
        let fcm_api_key = std::env::var("FCM_API_KEY")
            .map_err(|_| ErrorBuilder::new(
        ...
        if response.status().is_success() {
            ...
            ).details(error_text).build_error());
        }
...
impl OfflinePushSender for ApnsOfflinePushSender {
    async fn send(&self, task: &PushDispatchTask) -> Result<()> {
        ...
        let apns_token = task.metadata.get("apns_token")
            .ok_or_else(|| ErrorBuilder::new(
                ErrorCode::InvalidParameter,
        ...
        let apns_auth_key = std::env::var("APNS_AUTH_KEY")
            .map_err(|_| ErrorBuilder::new(
                ErrorCode::ConfigurationError,
...
impl OfflinePushSender for WebPushOfflinePushSender {
    async fn send(&self, task: &PushDispatchTask) -> Result<()> {
        ...
        let subscription = task.metadata.get("webpush_subscription")
            .ok_or_else(|| ErrorBuilder::new(
                ErrorCode::InvalidParameter,
        ...
        // 解析订阅信息
        let subscription_value: Value = serde_json::from_str(subscription)
            .map_err(|e| ErrorBuilder::new(
                ErrorCode::InvalidParameter,
                "Invalid WebPush subscription JSON",
            ).details(e.to_string()).build_error())?;
        
        let endpoint = subscription_value.get("endpoint")
            .and_then(|v| v.as_str())
            .ok_or_else(|| ErrorBuilder::new(
                ErrorCode::InvalidParameter,
