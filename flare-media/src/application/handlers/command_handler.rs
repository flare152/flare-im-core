...
use crate::infrastructure::media_processor::{MediaProcessor, ImageOperation, VideoOperation};
...
impl MediaCommandHandler {
    ...
    fn convert_image_operation(&self, operation: &flare_proto::media::ImageOperation) -> Result<crate::domain::model::MediaOperation> {
        ...
        let operation_type = match &operation.operation {
            ...
            Some(Compress(compress_op)) => {
                parameters.insert("quality".to_string(), Value::Number(compress_op.quality.into()));
                "compress".to_string()
            },
    ...
    fn convert_video_operation(&self, operation: &flare_proto::media::VideoOperation) -> Result<crate::domain::model::MediaOperation> {
        ...
        let operation_type = match &operation.operation {
            Some(Transcode(transcode_op)) => {
                parameters.insert("format".to_string(), Value::String(transcode_op.format.clone()));
                parameters.insert("quality".to_string(), Value::String(transcode_op.quality.clone()));
                "transcode".to_string()
            },
            Some(ExtractThumbnail(thumb_op)) => {
                parameters.insert("time".to_string(), Value::Number(serde_json::Number::from_f64(thumb_op.time).unwrap_or(serde_json::Number::from(0))));
                "extract_thumbnail".to_string()
            },
            Some(Compress(compress_op)) => {
                parameters.insert("bitrate".to_string(), Value::Number(compress_op.bitrate.into()));
                "compress".to_string()
            },
            Some(SubtitleBurn(subtitle_op)) => {
                parameters.insert("subtitle_file_id".to_string(), Value::String(subtitle_op.subtitle_file_id.clone()));
    ...
    async fn process_image_pipeline(&self, request: ProcessImageRequest) -> Result<ProcessedMediaResult> {
        info!(
            file_id = %request.file_id,
        ...
        // 1. 获取原始文件信息
        let original_file = self.domain_service.get_metadata(&request.file_id).await
        ...
        for operation in &request.operations {
            ...
            let operation_result = match media_operation.r#type.as_str() {
                "resize" => self.process_image_resize(&original_file, &media_operation).await?,
                "crop" => self.process_image_crop(&original_file, &media_operation).await?,
                "compress" => self.process_image_compress(&original_file, &media_operation).await?,
                "watermark" => self.process_image_watermark(&original_file, &media_operation).await?,
                "thumbnail" => self.process_image_thumbnail(&original_file, &media_operation).await?,
                _ => return Err(anyhow::anyhow!("Unsupported image operation: {}", media_operation.r#type)),
            };
            processing_results.push(operation_result);
        }

        // 3. 合并处理结果
        let processed_url = self.merge_processing_results(&request.file_id, &processing_results).await?;

        // 4. 更新文件元数据
        self.update_file_metadata(&request.file_id, &processing_results).await?;
    ...
    /// 5. 更新文件元数据
    #[instrument(skip(self), fields(file_id = %request.file_id))]
    async fn process_video_pipeline(&self, request: ProcessVideoRequest) -> Result<ProcessedMediaResult> {
        info!(
            file_id = %request.file_id,
            operations_count = request.operations.len(),
        ...
        for operation in &request.operations {
            ...
            let operation_result = match media_operation.r#type.as_str() {
                "transcode" => self.process_video_transcode(&original_file, &media_operation).await?,
                "extract_thumbnail" => self.process_video_thumbnail(&original_file, &media_operation).await?,
                "extract_audio" => self.process_video_audio_extraction(&original_file, &media_operation).await?,
                "compress" => self.process_video_compress(&original_file, &media_operation).await?,
                "watermark" => self.process_video_watermark(&original_file, &media_operation).await?,
                _ => return Err(anyhow::anyhow!("Unsupported video operation: {}", media_operation.r#type)),
            };
            processing_results.push(operation_result);
        }

        // 3. 合并处理结果
        let processed_url = self.merge_video_processing_results(&request.file_id, &processing_results).await?;

        // 4. 更新文件元数据
        self.update_video_metadata(&request.file_id, &processing_results).await?;
    ...
    /// 图片调整大小处理
    async fn process_image_resize(&self, file: &MediaFileMetadata, operation: &crate::domain::model::MediaOperation) -> Result<crate::domain::model::MediaProcessingResult> {
        // 构建图片操作参数
        let img_op = ImageOperation {
            operation_type: "resize".to_string(),
            width: operation.parameters.get("width").and_then(|v| v.as_i64()).map(|v| v as u32),
            height: operation.parameters.get("height").and_then(|v| v.as_i64()).map(|v| v as u32),
            quality: None,
            text: None,
            size: None,
        };
        ...
        // 执行图片处理
        let results = MediaProcessor::process_image(&temp_input_path, &temp_output_path, &[img_op]).await;
    ...
    /// 图片裁剪处理
    async fn process_image_crop(&self, file: &MediaFileMetadata, operation: &crate::domain::model::MediaOperation) -> Result<crate::domain::model::MediaProcessingResult> {
        // 构建图片操作参数
        let img_op = ImageOperation {
    ...
    /// 图片压缩处理
    async fn process_image_compress(&self, file: &MediaFileMetadata, operation: &crate::domain::model::MediaOperation) -> Result<crate::domain::model::MediaProcessingResult> {
        // 构建图片操作参数
        let img_op = ImageOperation {
            operation_type: "compress".to_string(),
            width: None,
            height: None,
            quality: operation.parameters.get("quality").and_then(|v| v.as_i64()).map(|v| v as u8),
            text: None,
            size: None,
        };
    ...
    /// 图片水印处理
    async fn process_image_watermark(&self, file: &MediaFileMetadata, operation: &crate::domain::model::MediaOperation) -> Result<crate::domain::model::MediaProcessingResult> {
        // 构建图片操作参数
        let img_op = ImageOperation {
            ...
            height: None,
            quality: None,
            text: operation.parameters.get("text").and_then(|v| v.as_str()).map(|s| s.to_string()),
            size: None,
        };
    ...
    /// 图片缩略图处理
    async fn process_image_thumbnail(&self, file: &MediaFileMetadata, operation: &crate::domain::model::MediaOperation) -> Result<crate::domain::model::MediaProcessingResult> {
        // 构建图片操作参数
        let img_op = ImageOperation {
            ...
            quality: None,
            text: None,
            size: operation.parameters.get("size").and_then(|v| v.as_i64()).map(|v| v as u32),
        };
    ...
    /// 视频转码处理
    async fn process_video_transcode(&self, file: &MediaFileMetadata, operation: &crate::domain::model::MediaOperation) -> Result<crate::domain::model::MediaProcessingResult> {
        // 构建视频操作参数
        let vid_op = VideoOperation {
            operation_type: "transcode".to_string(),
            format: operation.parameters.get("format").and_then(|v| v.as_str()).map(|s| s.to_string()),
            quality: operation.parameters.get("quality").and_then(|v| v.as_str()).map(|s| s.to_string()),
            time: None,
            bitrate: None,
            text: None,
        };
        ...
        // 执行视频处理
        let results = MediaProcessor::process_video(&temp_input_path, &temp_output_path, &[vid_op]).await;
    ...
    /// 视频缩略图处理
    async fn process_video_thumbnail(&self, file: &MediaFileMetadata, operation: &crate::domain::model::MediaOperation) -> Result<crate::domain::model::MediaProcessingResult> {
        // 构建视频操作参数
        let vid_op = VideoOperation {
    ...
    /// 视频音频提取处理
    async fn process_video_audio_extraction(&self, file: &MediaFileMetadata, _operation: &crate::domain::model::MediaOperation) -> Result<crate::domain::model::MediaProcessingResult> {
        // 构建视频操作参数
        let vid_op = VideoOperation {
    ...
    /// 视频压缩处理
    async fn process_video_compress(&self, file: &MediaFileMetadata, operation: &crate::domain::model::MediaOperation) -> Result<crate::domain::model::MediaProcessingResult> {
        // 构建视频操作参数
        let vid_op = VideoOperation {
            operation_type: "compress".to_string(),
            format: None,
            quality: None,
            time: None,
            bitrate: operation.parameters.get("bitrate").and_then(|v| v.as_i64()).map(|v| v as u32),
            text: None,
        };
    ...
    /// 视频水印处理
    async fn process_video_watermark(&self, file: &MediaFileMetadata, operation: &crate::domain::model::MediaOperation) -> Result<crate::domain::model::MediaProcessingResult> {
        // 构建视频操作参数
        let vid_op = VideoOperation {
            operation_type: "watermark".to_string(),
            format: None,
    ...
    /// 合并图片处理结果
    async fn merge_processing_results(&self, file_id: &str, results: &[crate::domain::model::MediaProcessingResult]) -> Result<String> {
        // 选择最终的处理结果URL
        if let Some(last_result) = results.last() {
            if !last_result.output_url.is_empty() {
                Ok(last_result.output_url.clone())
            } else {
                Ok(format!("https://cdn.example.com/media/processed/{}/{}", file_id, last_result.operation_type))
            }
        } else {
            Ok(format!("https://cdn.example.com/media/processed/{}", file_id))
        }
    }

    /// 合并视频处理结果
    async fn merge_video_processing_results(&self, file_id: &str, results: &[crate::domain::model::MediaProcessingResult]) -> Result<String> {
        // 选择最终的处理结果URL
        if let Some(last_result) = results.last() {
            if !last_result.output_url.is_empty() {
                Ok(last_result.output_url.clone())
            } else {
                Ok(format!("https://cdn.example.com/media/video/processed/{}/{}", file_id, last_result.operation_type))
            }
        } else {
            Ok(format!("https://cdn.example.com/media/video/processed/{}", file_id))
        }
    }

    /// 更新文件元数据
    async fn update_file_metadata(&self, file_id: &str, results: &[crate::domain::model::MediaProcessingResult]) -> Result<()> {
        // 更新文件元数据逻辑
        tracing::info!(
            file_id = %file_id,
    ...
    /// 更新视频文件元数据
    async fn update_video_metadata(&self, file_id: &str, results: &[crate::domain::model::MediaProcessingResult]) -> Result<()> {
        // 更新视频文件元数据逻辑
        tracing::info!(
            file_id = %file_id,
            operations_count = results.len(),
    ...
    /// 清理临时文件
    async fn cleanup_temporary_files(&self, results: &[crate::domain::model::MediaProcessingResult]) -> Result<()> {
        // 清理临时文件逻辑
