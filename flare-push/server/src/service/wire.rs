...
pub async fn initialize(
    ...
    let session_client = if let Some(service_client) = session_service_client {
        ...
        tracing::warn!("Session service discovery not configured, get_all_online_users_for_session will not work");
        None
    };
    ...
    let gateway_router = if access_gateway_discover.is_some() {
        ...
        let discover_for_client = flare_im_core::discovery::create_discover(&access_gateway_service)
            .await
            .map_err(|e| anyhow::anyhow!("Failed to create discover for ServiceClient: {}", e))?
            .ok_or_else(|| anyhow::anyhow!("Service discovery not configured for Access Gateway service"))?;
        let discover_for_router = flare_im_core::discovery::create_discover(&access_gateway_service)
            .await
            .map_err(|e| anyhow::anyhow!("Failed to create discover for Gateway Router: {}", e))?
            .ok_or_else(|| anyhow::anyhow!("Service discovery not configured for Access Gateway service"))?;
        
        let service_client = flare_server_core::discovery::ServiceClient::new(discover_for_client);
        GatewayRouter::with_service_client_and_discover(gateway_router_config, service_client, discover_for_router)
