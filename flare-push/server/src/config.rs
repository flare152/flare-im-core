...
pub struct PushServerConfig {
    ...
    pub gateway_endpoints: String, // 格式: "gateway-1:http://localhost:50051,gateway-2:http://localhost:50052"
    // Signaling Online配置
...
impl PushServerConfig {
    pub fn from_app_config(app: &FlareAppConfig) -> Self {
        ...
        // Access Gateway gRPC 端口 = WebSocket端口 + 2
        // 默认 WebSocket 端口是 60051，所以 gRPC 端口是 60053
        let gateway_endpoints = env::var("GATEWAY_ENDPOINTS")
            .unwrap_or_else(|_| "local:http://localhost:60053".to_string());

        // Signaling Online配置
        let signaling_endpoint = env::var("SIGNALING_ENDPOINT")
