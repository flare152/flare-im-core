...
impl AckMetrics {
    ...
    pub fn new(registry: &Registry) -> Result<Self, Box<dyn std::error::Error>> {
        let total_acks_processed = IntCounter::new(
            "ack_total_processed",
            "Total number of ACKs processed"
        )?;
        
        let acks_persisted = IntCounter::new(
            "ack_persisted",
            "Number of ACKs persisted to storage"
        )?;
        
        let acks_cached = IntCounter::new(
            "ack_cached",
            "Number of ACKs cached in memory"
        )?;
        ...
        let batch_queue_size = IntGauge::new(
            "ack_batch_queue_size",
        ...
        let acks_by_type = IntCounterVec::new(
            Opts::new(
                "ack_processed_by_type",
                "Number of ACKs processed by type"
            ),
            &["type"]
        )?;
        ...
        let ack_timeouts = IntCounter::new(
            "ack_timeouts_total",
            "Total number of ACK timeouts"
        )?;
        
        let ack_retries = IntCounter::new(
            "ack_retries_total",
            "Total number of ACK retries"
        )?;
    ...
    /// 记录ACK处理
    pub fn record_ack_processed(&self, persisted: bool, cached: bool, importance: &str, ack_type: &str) {
        ...
        self.acks_by_importance.with_label_values(&[importance]).inc();
        self.acks_by_type.with_label_values(&[ack_type]).inc();
    }
    
    /// 记录ACK处理延迟
    pub fn record_ack_processing_latency(&self, ack_type: &str, duration: f64) {
        self.ack_processing_latency.with_label_values(&[ack_type]).observe(duration);
    }
    
    /// 记录不同重要性级别的处理延迟
    pub fn record_ack_processing_latency_by_importance(&self, importance: &str, duration: f64) {
        self.ack_processing_latency_by_importance.with_label_values(&[importance]).observe(duration);
    }
    ...
    pub fn record_redis_operation_latency(&self, operation: &str, duration: f64) {
        self.redis_operation_latency.with_label_values(&[operation]).observe(duration);
    }
    ...
    /// 获取指标数据
    pub fn get_metrics_data(&self, registry: &Registry) -> Result<String, Box<dyn std::error::Error>> {
...
impl AckPerformanceOptimizer {
    ...
    /// 优化ACK数据大小
    pub fn optimize_ack_data_size(&self, data: &[u8]) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
        if self.config.enable_compression && data.len() > self.config.compression_threshold {
            // 使用压缩算法优化数据大小
            let compressed = zstd::encode_all(data, 3)?;
            self.metrics.record_ack_processed(false, true, "high", "compressed");
            Ok(compressed)
        } else {
            Ok(data.to_vec())
        }
