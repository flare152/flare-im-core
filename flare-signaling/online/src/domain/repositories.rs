...
use crate::domain::entities::{DeviceInfo, OnlineStatusRecord, SessionRecord};
...
pub trait PresenceWatcher: Send + Sync {
    /// 监听用户在线状态变化
    async fn watch_presence(&self, user_ids: &[String]) -> Result<tokio::sync::mpsc::Receiver<Result<PresenceChangeEvent>>>;
}
