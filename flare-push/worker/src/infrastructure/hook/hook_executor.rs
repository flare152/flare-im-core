...
use flare_proto::hooks::{
    DeliveryHookRequest, DeliveryHookResponse,
    PushPreSendHookRequest, PushPreSendHookResponse,
    PushPostSendHookRequest, PushPostSendHookResponse,
    HookInvocationContext, HookDeliveryEvent as ProtoHookDeliveryEvent,
};
use flare_im_core::hooks::{DeliveryEvent, HookContext, MessageRecord, MessageDraft};
    ...
    fn delivery_event_to_proto(event: &DeliveryEvent) -> ProtoHookDeliveryEvent {
        ProtoHookDeliveryEvent {
            ...
            delivered_at: Some(prost_types::Timestamp {
                seconds: event.delivered_at.duration_since(std::time::UNIX_EPOCH)
            ...
            match HookExtensionClient::connect(endpoint).await {
                Ok(mut new_client) => {
                    ...
                    match tokio::time::timeout(
                        self.timeout,
                        new_client.notify_delivery(request)
                    ).await {
                        Ok(Ok(response)) => {
    ...
    pub async fn push_pre_send(
        ...
        if let Some(client) = &self.client {
            ...
            let hook_draft = flare_proto::hooks::HookPushDraft {
                ...
                content: push_request.message.as_ref()
                    .and_then(|m| m.content.as_ref())
                    .map(|c| format!("{:?}", c)) // 简单格式化内容
                    .unwrap_or_default(),
                channel: "push".to_string(), // 默认渠道
                message_id: push_request.message.as_ref().map(|m| m.id.clone()).unwrap_or_default(),
        ...
        if let Some(client) = &self.client {
            ...
            let hook_record = flare_proto::hooks::HookPushRecord {
                ...
                enqueued_at: Some(prost_types::Timestamp {
                    seconds: record.persisted_at.duration_since(std::time::UNIX_EPOCH)
                        .map(|d| d.as_secs() as i64)
                        .unwrap_or(0),
                    nanos: 0,
                }),
            ...
            tokio::spawn(async move {
                match HookExtensionClient::connect(endpoint).await {
                    Ok(mut new_client) => {
                        match new_client.invoke_push_post_send(request).await {
                            Ok(response) => {
                                let response = response.into_inner();
                                if let Some(status) = response.status {
                                    if status.code != 0 {
