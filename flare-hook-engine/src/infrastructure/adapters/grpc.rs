...
use flare_proto::hooks::{
    PreSendHookRequest, PostSendHookRequest, DeliveryHookRequest, RecallHookRequest,
};
use flare_im_core::{
...
impl GrpcHookAdapter {
    ...
    pub async fn pre_send(
        ...
        let response = client.invoke_pre_send(request)
            .await
    ...
    pub async fn post_send(
        ...
        let response = client.invoke_post_send(request)
            .await
        ...
        if response.success {
            ...
            let error = response.status
                ...
                .unwrap_or_else(|| {
                    ErrorBuilder::new(ErrorCode::InternalError, "PostSend hook failed").build_error()
                });
            Err(error.into())
        }
    }
    ...
    pub async fn delivery(
        ...
        let response = client.notify_delivery(request)
            .await
        ...
        if response.success {
            ...
            let error = response.status
                ...
                .unwrap_or_else(|| {
                    ErrorBuilder::new(ErrorCode::InternalError, "Delivery hook failed").build_error()
                });
            Err(error.into())
        }
    }
    ...
    pub async fn recall(
        ...
        let response = client.notify_recall(request)
            .await
