...
use crate::domain::models::{
    ...
    SessionFilter, SessionLifecycleState, SessionParticipant, SessionPolicy, SessionSort,
                        ...
                        if let Some(last_msg) = sync_result.messages.first() {
                            ...
                            if let Some(ref content) = last_msg.content {
                                summary.last_content_type = match &content.content {
                                    Some(flare_proto::storage::message_content::Content::Text(_)) => {
                                        Some("text".to_string())
                                    }
                                    Some(flare_proto::storage::message_content::Content::Binary(_)) => {
                                        Some("binary".to_string())
                                    }
                                    Some(flare_proto::storage::message_content::Content::Json(_)) => {
                                        Some("json".to_string())
                                    }
