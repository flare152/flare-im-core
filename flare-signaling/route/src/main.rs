...
use flare_signaling_route::interface::grpc::SignalingRouteServer;
...
async fn main() -> Result<()> {
    ...
    register_service(&runtime_config, "signaling-route")
        .await
        .map_err(|err| {
    ...
    let service = SignalingRouteServer::new(runtime_config.clone()).await?;

    info!("Starting Signaling Route Service on {}", addr);
