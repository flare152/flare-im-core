...
impl MessagePersistenceCommandHandler {
    ...
    pub async fn handle(&self, command: ProcessStoreMessageCommand) -> Result<PersistenceResult> {
        ...
        // 验证并补全媒资附件
        if let Err(e) = self.domain_service.verify_and_enrich_media(&mut prepared.message).await {
    ...
    pub async fn handle_batch(&self, commands: Vec<ProcessStoreMessageCommand>) -> Result<Vec<PersistenceResult>> {
        let start = Instant::now();
        ...
        if !new_messages.is_empty() {
            let db_start = Instant::now();
            match self.domain_service.persist_batch(new_messages.clone()).await {
                Ok(_) => {
                    let db_duration = db_start.elapsed();
                    self.metrics.db_write_duration_seconds.observe(db_duration.as_secs_f64());
        ...
        for prepared in &prepared_messages {
            let deduplicated = deduplicated_count > 0 && !new_messages.iter().any(|m| m.message_id == prepared.message_id);
