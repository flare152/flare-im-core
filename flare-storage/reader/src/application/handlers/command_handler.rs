...
impl MessageStorageCommandHandler {
    ...
    pub async fn handle_delete_message(&self, command: DeleteMessageCommand) -> Result<usize> {
        self.domain_service
            .delete_messages(&command.message_ids)
            .await
    }
    ...
    pub async fn handle_set_attributes_with_operation(
        ...
        self.domain_service
            .append_operation_and_attributes(
                &command.message_id,
                operation,
                command.attributes,
                command.tags,
    ...
    pub async fn handle_export_messages(&self, command: ExportMessagesCommand) -> Result<String> {
        ...
        tokio::spawn(async move {
            // 执行导出逻辑
            if let Err(e) =
                Self::execute_export_task(domain_service, command_clone, &export_task_id_clone)
                    .await
            {
                tracing::error!(
